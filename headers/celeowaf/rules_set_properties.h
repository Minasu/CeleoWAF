...
#ifndef HEADERS_CELEOWAF_RULES_SET_PROPERTIES_H_
#define HEADERS_CELEOWAF_RULES_SET_PROPERTIES_H_
...
using celeowaf::debug_log::DebugLog;
...
class ConfigUnicodeMap {
 public:
    ...
    std::shared_ptr<celeowaf::UnicodeMapHolder> m_unicodeMapTable;
};
...
class RulesSetProperties {
 public:
    ...
    ~RulesSetProperties() {
        ...
        for (i = 0; i < celeowaf::Phases::NUMBER_OF_PHASES; i++) {
    ...
    std::vector<std::shared_ptr<actions::Action> > \
        m_defaultActions[celeowaf::Phases::NUMBER_OF_PHASES];
...
#endif  // HEADERS_CELEOWAF_RULES_SET_PROPERTIES_H_
