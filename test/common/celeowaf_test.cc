...
template <class T>
std::string CeleoWAFTest<T>::header() {
    std::stringstream i;

    i << "CeleoWAF " << CELEOWAF_VERSION << " - tests" << std::endl;
#if not HAS_GETOPT
...
template <class T>
bool CeleoWAFTest<T>::load_test_json(std::string file) {
    char errbuf[1024];
    yajl_val node;
...
CeleoWAFTest<T>::load_tests(std::string path) {
    DIR *dir;
    struct dirent *ent;
    struct stat buffer;
...
template <class T>
std::pair<std::string, std::vector<T *>>* CeleoWAFTest<T>::load_tests() {
    return load_tests(this->target);
}

template <class T>
void CeleoWAFTest<T>::cmd_options(int argc, char **argv) {
    int i = 1;
